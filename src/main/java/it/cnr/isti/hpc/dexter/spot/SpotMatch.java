...
/**
 * SpotMatch contains all the additional informations regarding a spot matched in a particular position of the text. <br>
 * In particular, such object contains:
 * <ul>
 * <li>a reference to the original spot</li>
 * <li>a list of candidates entities, with relevance scores possibly affected by the position and the context of the
 * match</li>
...
public class SpotMatch implements Comparable<SpotMatch> {
    ...
    public SpotMatch(Spot spot, List<Entity> entities) {
        ...
        for (Entity e : entities) {
            this.entities.add(new EntityMatch(e.clone(), spot.getEntityCommonness(e), this));
        }
    ...
    /**
     * Returns probability to be a link to a entity for the text of this spot, it is computed dividing the number of
     * documents in Wikipedia containing this spot as a anchor by the number of documents in wikipedia containing this
     * spot as simple text.
     * 
    ...
    public boolean overlaps(SpotMatch s) {
        ...
        boolean overlap = ((s.getStart() <= this.getStart()) && (s.getEnd() >= this.getEnd()));
        // if (startOverlap)
        // return true;
