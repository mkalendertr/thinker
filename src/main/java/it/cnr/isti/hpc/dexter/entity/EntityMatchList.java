...
public class EntityMatchList extends ArrayList<EntityMatch> {
	...
	public int entityIndex(String e) {
		int pos = 0;
		for (EntityMatch em : this) {
			if (em.getId().equals(e)) {
				return pos;
			}
			pos++;
		}
	...
	public EntityMatchList removeOverlappings() {
		...
		for (EntityMatch e : this) {
			...
			for (EntityMatch e1 : this) {
				// check if e overlaps with some entity yet in the result list
				if (e1.getId() != e.getId()) {
					overlaps = StringUtils.containsIgnoreCase(e1.getSpot()
							.getMention(), e.getSpot().getMention());
					// overlaps = e.overlaps(e1);
					if (overlaps) {
						e.setEntity(e1.getEntity());
						break;
					}
				}
	...
	public EntityMatchList removeMultiMeanings() {

		EntityMatchList eml = new EntityMatchList();
		for (EntityMatch e : this) {
			for (EntityMatch e1 : this) {
				// check if e overlaps with some entity yet in the result list
				if (e1.getMention().equals(e.getMention())
						&& e1.getId() != e.getId()) {

					if (e1.getScore() > e.getScore()) {
						e.setEntity(e1.getEntity());

					} else {
						e1.setEntity(e.getEntity());
