...
public class Spot implements Serializable {
	...
	public static Spot fromByteArray(String text, byte[] data) {
		...
		for (int i = 2; i < array.length; i += 2) {
			entities.add(new Entity(String.valueOf(array[i]), array[i + 1]));
		}
	...
	public static Spot fromTsvLine(String text) {
		...
		// List<String> entities = new StringtArrayString(false).toArray(scanner
		// .next());
		// List<Integer> freqs = new
		// IntArrayString(false).toArray(scanner.next());
