...
public class SentenceSegmenter {
     ...
     */
    private static final Logger logger = LoggerFactory.getLogger(SentenceSegmenter.class);
...
//            modelIn = new FileInputStream(file);
            InputStream fis = new FileInputStream("nlp"+File.separator+"en-sent.bin");
            final SentenceModel sentenceModel = new SentenceModel(fis);
            fis.close();
    ...
    public List<Sentence> splitPos(String text) {
        ...
        for (Span s : sentenceDetector.sentPosDetect(text)) {
            ...
            sentences.add(new Sentence(text.substring(s.getStart(), s.getEnd()), s.getStart(), s.getEnd()));
    ...
    public static void main(String[] args) {
        String test =
            "France deployed two fighter jets today to search for a missing airliner with 116 people aboard that has reportedly crashed after flying into a violent sandstorm in the Sahara desert. Air Algerie flight 5017 took off in the early hours from Ouagadougou";
