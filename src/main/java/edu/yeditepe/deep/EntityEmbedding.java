...
public class EntityEmbedding {
	private static final Logger LOGGER = Logger
			.getLogger(EntityEmbedding.class);
	...
	private static String modelFile = "embedding_network_test.model1";
	...
	public static void main(final String args[]) {
		EntityEmbedding ee = new EntityEmbedding();
	...
	public BasicNetwork train() {
		...
		if (Boolean.parseBoolean(Property.getInstance().get(
				"embedding.autoencoder"))) {
			EntityEmbeddingAutoencoder ee = new EntityEmbeddingAutoencoder();
			network = ee.preTrain();
			createEmbeddingsFile("autoencoder_resilient");
			ee = null;
		...
		for (int z = 0; z < epoch; z++) {
			...
			// EncogDirectoryPersistence.saveObject(new File(modelFile + (z +
			// 1)),
			// network);
	...
	private void createEmbeddingsFile(String prefix) {
		...
		Writer writer;
		try {
			writer = new FileWriter(prefix + "embeddings_new.json");
